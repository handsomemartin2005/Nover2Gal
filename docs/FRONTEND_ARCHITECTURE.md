...
- `/account` 提供注册、登录、资料修改和密码修改；会话使用 `HttpOnly`、`SameSite=Lax` Cookie，生产环境必须通过 HTTPS 并启用 `SESSION_COOKIE_SECURE=true`。
- `/admin` 仅管理员可访问，用于用户启停、角色调整、项目/样例归属转移、公开状态管理和内容删除。
- 项目始终归创建者私有；普通项目 API 只允许所有者读取，包括管理员也不能通过该 API 查看他人的原文。管理 API 仅返回项目元数据。
- 私密样例只对所有者可见。公开样例可被访客浏览，但发布为公开时会强制清除原文全文；其他用户必须登录后才能复制公开样例。
- 用户、密码哈希和会话保存在 `AUTH_DB_PATH` 指向的 SQLite 数据库中。密码采用 PBKDF2-SHA256 加盐哈希，会话数据库只保存令牌哈希。
- 首次升级可用 `NOVEL2GAL_ADMIN_USERNAME`、`NOVEL2GAL_ADMIN_PASSWORD` 创建管理员，并用 `NOVEL2GAL_CLAIM_LEGACY_TO_ADMIN=true` 将无归属旧数据收归管理员。首次启动成功后应立即删除明文引导密码和迁移标志。
