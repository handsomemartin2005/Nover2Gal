...
import { loadCurrentUser } from "/static/js/auth-state.js?v=20260710-auth2";
...
async function loadAdminState() {
  const [overview, users, projects, samples] = await Promise.all([
    api.adminOverview(), api.adminUsers(), api.adminProjects(), api.adminSamples(),
  ]);
  return { overview, users: users.users || [], projects: projects.projects || [], samples: samples.samples || [] };
}
...
function render(host, state, currentUser) {
  host.innerHTML = `
    <section class="admin-dashboard">
      ...
      <section class="admin-stats">${stat("用户", state.overview.users)}${stat("项目", state.overview.projects)}${stat("公开样例", state.overview.public_samples)}${stat("私密样例", state.overview.private_samples)}${stat("未归属", state.overview.unowned_projects)}</section>
      <nav class="admin-tabs"><button class="active" type="button" data-admin-tab="users">用户</button><button type="button" data-admin-tab="projects">项目</button><button type="button" data-admin-tab="samples">样例与公开状态</button></nav>
